...
Route::post('/send-notification', function (Request $request) {
    $userId = $request->user_id; // معرف المستخدم الذي سيستلم الإشعار
    $message = $request->message; // الرسالة المراد إرسالها

    // إطلاق الحدث
    event(new \App\Events\UserNotificationEvent($userId, $message));
