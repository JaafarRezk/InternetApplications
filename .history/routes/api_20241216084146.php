...
Route::middleware('auth:sanctum')->post('/send-notification', function (Request $request) {
    $userId = auth()->user()->id;
    $message = $request->input('message', "Hello User $userId, this is a test notification!");

    // تخزين الإشعار في الجلسة
    session()->push("notifications.user_$userId", $message);

    broadcast(new \App\Events\UserNotificationEvent($message, $userId));

    return response()->json(['message' => 'Notification sent successfully.']);
});


    // إضافة Route لتحديد الإشعارات كمقروءة
    Route::post('/notifications/mark-as-read', function (Request $request) {
        $userId = auth()->user()->id;
        // تحديث جميع الإشعارات للمستخدم كمقروءة
