...
class FilePolicy
{
    ...
    public function downloadFile(User $user, File $file): bool
    {
        ...
        return false; // Prevent download if the file is not checked in
    }

    public function downloadFile(User $user, File $file): bool
{
    // السماح بالتنزيل فقط إذا كان الملف في حالة Check-In من قبل المستخدم الحالي
    return $file->checked == 0 || $file->file_holder_id === $user->id;
}
