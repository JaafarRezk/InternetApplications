...
class ReportService extends Service
{
    ...
    public function exportUserOperationsReport($groupId)
    {
        ...
        // جلب العمليات المرتبطة بالملفات ضمن المجموعة
        $logs = Log::whereHas('file', function ($query) use ($group) {
            $query->whereHas('groups', function ($groupQuery) use ($group) {
                $groupQuery->where('group_id', $group->id);
            });
        })
        ->with(['file', 'file.groups', 'user']) // جلب البيانات المرتبطة
        ->get();
