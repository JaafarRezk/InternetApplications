...
public function checkInMultipleFiles(array $fileIds)
{
    ...
    foreach ($fileIds as $fileId) {
        ...
        $lock = Cache::lock($lockKey, 10); // مدة القفل 10 ثوانٍ

        if ($lock->get()) {
            try {
                // استرجاع الملف مع التحقق
                $file = File::fetchByIdWithCacheAndAuth($fileId);

                if ($file->checked != 0) {
                    throw new FileInUseException("File ID {$file->id} is in use and cannot be checked in.");
                }

                // إنشاء نسخة جديدة للملف
                $latestVersion = $file->versions()->orderBy('created_at', 'desc')->first();
                $file->versions()->create([
                    'file_id' => $file->id,
                    'name' => $file->name,
            ...
            throw new FileInUseException("File ID {$fileId} is being modified by another user.");
        }
    }
...
public function getFileVersions($fileId)
{
    ...
    // جلب الإصدارات المرتبطة بالملف
    $versions = $file->versions()->orderBy('version_number', 'desc')->get();
