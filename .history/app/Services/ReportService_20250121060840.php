...
class ReportService extends Service
{
    ...
    {
        $group = $this->validateGroupAccess($groupId); // يمكن لجميع الأعضاء الوصول للتقارير
        
        // الحصول على السجلات الخاصة بالملفات المرتبطة بالمجموعة
        $logs = FileLog::whereHas('file.groups', function ($query) use ($group) {
            $query->where('group_id', $group->id);
        })->get();
        
        // إرجاع السجلات الخاصة بالعمليات التي تمت على الملفات
        return $logs;
    }
    ...
    {
        $group = $this->validateGroupCreator($groupId); // فقط منشئ المجموعة يمكنه الوصول
        
        // الحصول على السجلات الخاصة بالعمليات التي قام بها المستخدمون في مجموعة معينة
        $logs = Log::whereHas('file.groups', function ($query) use ($group) {
            $query->where('group_id', $group->id);
        })->get()->groupBy('user_id');
        
        // إرجاع السجلات مجمعة حسب المستخدمين
        return $logs;
    }
