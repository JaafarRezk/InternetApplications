...
class ReportService extends Service
{
    ...
    {
        // التحقق من الوصول إلى المجموعة
        $group = $this->validateGroupCreator($groupId); // التحقق من أن المستخدم هو منشئ المجموعة
    
        // جلب العمليات التي قام بها المستخدمون ضمن المجموعة
        $logs = Log::whereHas('file.groups', function ($query) use ($group) {
            $query->where('group_id', $group->id); // التحقق من أن العملية مرتبطة بملف ضمن المجموعة
        })
        ->with(['file', 'user']) // جلب بيانات المستخدم والملف
        ->get();
    
        return $logs->groupBy('user_id'); // تجميع العمليات بناءً على المستخدم
    }
