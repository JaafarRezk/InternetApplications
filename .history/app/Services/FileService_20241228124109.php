...
{
    // الحصول على الملف من قاعدة البيانات مع التحقق من الصلاحيات
    $file = File::fetchByIdWithCacheAndAuth($fileId);

    if (!$file) {
        throw new ObjectNotFoundException('الملف غير موجود.');
    }

    // التحقق من صلاحية المستخدم للوصول إلى الملف
    if (!Gate::allows('viewFile', $file)) {
        throw new AuthorizationException('غير مسموح لك بتنزيل هذا الملف.');
    }
