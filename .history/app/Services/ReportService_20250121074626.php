...
class ReportService extends Service
{
    ...
    public function exportUserOperationsReport($groupId)
    {
        ...
        // جلب المستخدمين في المجموعة مع العمليات التي قاموا بها
        $usersWithLogs = $group->users()
            ->with(['logs' => function ($query) use ($group) {
                $query->whereHas('file.groups', function ($groupQuery) use ($group) {
                    $groupQuery->where('group_id', $group->id); // العمليات المرتبطة بملفات في هذه المجموعة
                })
                ->with(['file']); // جلب تفاصيل الملفات المرتبطة
            }])
            ->get();
    
        // تعديل هيكل البيانات بحيث يتم تضمين السجلات الخاصة بكل مستخدم مع البيانات
        $userReports = $usersWithLogs->map(function ($user) {
            return [
                'id' => $user->id,
                'name' => $user->name,
                'email' => $user->email,
                'logs' => $user->logs->map(function ($log) {
                    return [
                        ...
                        'status' => $log->status,
                        'date' => $log->created_at->toDateTimeString(), // تاريخ السجل
                        'user_id' => $log->user_id,
                        'file_id' => $log->file_id,
                        'file_name' => $log->file ? $log->file->name : null, // اسم الملف
                    ];
                }),
            ];
        });
        ...
        return response()->json([
            'success' => true,
            'data' => $userReports,
            'message' => 'api.report.exportUserOperationsReport.success',
        ]);
    }
