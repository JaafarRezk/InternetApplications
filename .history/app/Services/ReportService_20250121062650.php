...
class ReportService extends Service
{
    ...
    {
        // تحقق من صلاحيات الوصول إلى المجموعة
        $group = $this->validateGroupAccess($groupId); 
    
        // جلب العمليات على الملفات ضمن المجموعة
        $logs = FileLog::whereHas('file.groups', function ($query) use ($group) {
            $query->where('group_id', $group->id); // التأكد من أن الملف مرتبط بالمجموعة
        })
        ->with(['file', 'user']) // جلب بيانات الملف والمستخدم المرتبطة
        ->get();
