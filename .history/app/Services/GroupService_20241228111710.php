...
public function respondToFileAdditionRequest($parameters)
{
    try {
        ...
        // فك ترميز معرفات الملفات من البيانات المخزنة
        $filesIds = json_decode($parameters['files_ids'], true);

        if ($parameters['response'] === 'accept') {
            // إضافة الملفات إلى المجموعة مباشرة بدون التحقق من الملكية
            $group->files()->syncWithoutDetaching($filesIds);

            // إرسال إشعار للمستخدم الذي طلب إضافة الملفات
            Notification::createNewWithValidation([
                'user_id' => $parameters['requester_user_id'], // المستخدم الذي طلب إضافة الملفات
                'group_id' => $group->id,
...
{
    // جلب جميع المجموعات التي انضم إليها المستخدم بناءً على ID المستخدم
    $groups = Group::whereHas('users', function ($query) use ($userId) {
        $query->where('users.id', $userId);
    })->get(['id', 'name', 'creator_id']);

    // يمكنك إضافة تفاصيل إضافية مثل عدد الأعضاء أو الملفات
    $groupsWithDetails = $groups->map(function ($group) {
        $group->file_count = $group->files()->count();  // عدد الملفات في المجموعة
        $group->user_count = $group->users()->count();  // عدد الأعضاء في المجموعة
        return $group;
    });
