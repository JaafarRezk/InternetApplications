...
{
    try {
        // تمرير المعرّف إلى واجهة FileFacade
        $fileFacade = new FileFacade(['urlParameters' => ['id' => $id]]);
        return $fileFacade->downloadFile(); // تحميل الملف
    } catch (\Exception $e) {
        return response()->json(['error' => $e->getMessage()], 400); // إرجاع خطأ في حالة الفشل
    }
}
