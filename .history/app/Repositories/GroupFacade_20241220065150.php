...
class GroupFacade extends Facade
{
    ...
    {
        $groupId = $this->message['bodyParameters']['group_id']; // ID المجموعة
        $invitedUserId = $this->message['bodyParameters']['invited_user_id']; // ID المستخدم المدعو

        // التحقق من صحة البيانات (اختياري)
        if (empty($groupId) || empty($invitedUserId)) {
            throw new \Exception('Invalid group ID or invited user ID');
        }

        // استدعاء الخدمة لإرسال الدعوة
